  ...
  return (
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled || menuOpen ? 'bg-[#faf9f6]/95 backdrop-blur-md border-b border-stone-200/50 shadow-sm' : 'bg-transparent'} ${isScrolled ? 'py-1' : 'py-2'}`}
      >
        <nav className="w-full px-6 md:px-6 lg:px-16 h-20 flex items-center justify-between md:grid md:grid-cols-3">
            ...
            {navLinks.map((link) => (
              <li key={link.href}>
                <button
                  ...
                  className="text-[13px] font-medium tracking-wide text-stone-700 hover:text-[#d565d9] transition-colors duration-200 py-1"
                >
          ...
          <div className="flex justify-end items-center gap-2">
            <motion.a
              ...
              whileTap={{ scale: 0.97 }}
              className="hidden md:flex group items-center gap-3 pl-1.5 pr-5 py-1.5 rounded-full bg-[#d565d9] border border-[#d565d9] hover:bg-[#b04cad] transition-colors duration-300 shadow-md decoration-0"
            >
            ...
            <button
              className="md:hidden flex items-center justify-center w-[38px] h-[38px] rounded-full bg-[#d565d9] text-white hover:bg-[#b04cad] transition-colors flex-shrink-0 shadow-sm"
